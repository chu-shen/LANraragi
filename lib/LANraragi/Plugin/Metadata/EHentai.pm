...
sub plugin_info {
    ...
    return (
        ...
        parameters => [
            { type => "string", desc => "Forced custom filters to use in searches (Japanese won't work due to EH limitations)" },
            { type => "bool",   desc => "Save archive title. Independent" },
...
sub get_tags {
    ...
    my ( $customFilters, $savetitle, $usethumbs, $search_gid, $enablepanda, $jpntitle, $additionaltags, $expunged ) = @_; # Plugin parameters
    ...
    if ( $lrr_info->{oneshot_param} =~ /.*\/g\/([0-9]*)\/([0-z]*)\/*.*/ ) {
        ...
        ( $gID, $gToken ) = &lookup_gallery(
            ...
            $ua, $domain, $customFilters, $usethumbs, $search_gid, $expunged
        );
    }
...
sub lookup_gallery {
    ...
    my ( $title, $tags, $thumbhash, $ua, $domain, $customFilters, $usethumbs, $search_gid, $expunged ) = @_;
    my $logger = get_plugin_logger();
    my $URL    = "";
    ...
    # TODO Even if the conditions below are removed, the filter still does not work properly.
    ...
    # Add the custom filters override, if it's defined.
    if ( $customFilters ne "" ) {
        $URL = $URL . "+" . uri_escape_utf8("$customFilters");
    }
